...
|   [ ]    | ネットワークACL          | NACL             | サブネットレベルのファイアウォール       |
...
|   [x]    | サーバーレス       | Upload Lambda         | API Gateway経由のリクエスト処理（VPC外）        |
|   [x]    | サーバーレス       | RDS Lambda            | OpenSearch検索・RDS詳細取得（VPC内）            |
|   [x]    | サーバーレス       | Sync Lambda           | S3差分ファイル取得・RDS/OpenSearch更新（VPC内） |
|   [x]    | サーバーレス       | Tavily&Bedrock Lambda | Bedrock連携・検索API呼び出し                    |
...
|   [ ]    | 通知             | SNS         | アラート通知（Slack連携等）   |

#### 認証・認可

| チェック | カテゴリ     | リソース名 | 用途・備考                      |
| :------: | ------------ | ---------- | ------------------------------- |
|   [ ]    | 認証         | Cognito    | ユーザー認証・トークン管理      |
|   [ ]    | 認可         | IAM        | AWSリソースへのアクセス制御     |
|   [ ]    | シークレット | Secrets Manager | DBパスワード・APIキー管理   |
